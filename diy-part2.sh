...
# 引入日志函数（注意：此处直接写，因环境可能无函数定义）
OPT_LOG_FILE="${OPT_LOG_FILE:-/var/log/opt.log}"
DEBUG_OPT="${DEBUG_OPT:-0}"
log_opt() {
    local msg="$1"
    ...
    ext4|btrfs|xfs|f2fs|zfs)
        ...
        if [ -n "$DEV_BASE" ] && [ -f "/sys/block/$DEV_BASE/queue/read_ahead_kb" ]; then
            if [ "$REMOVABLE" = "1" ]; then
                ...
                echo "${SSD_READ_AHEAD_KB:-2048}" > "/sys/block/$DEV_BASE/queue/read_ahead_kb" 2>/dev/null && \
                    log_opt "SSD $DEV_BASE 预读缓存设为 ${SSD_READ_AHEAD_KB:-2048}KB"
            else
                echo "${HDD_READ_AHEAD_KB:-128}" > "/sys/block/$DEV_BASE/queue/read_ahead_kb" 2>/dev/null && \
                    log_opt "HDD $DEV_BASE 预读缓存设为 ${HDD_READ_AHEAD_KB:-128}KB"
            fi
        fi
        ...
        if [ -n "$DEV_BASE" ] && [ -f "/sys/block/$DEV_BASE/queue/scheduler" ]; then
            if [ "$REMOVABLE" = "1" ]; then
                ...
                echo "none" > "/sys/block/$DEV_BASE/queue/scheduler" 2>/dev/null || \
                echo "noop" > "/sys/block/$DEV_BASE/queue/scheduler" 2>/dev/null && \
                    log_opt "USB $DEV_BASE I/O 调度器设为 none/noop"
            elif [ "$rotational" = "0" ]; then
                # SSD: 使用 none 或 noop
                echo "none" > "/sys/block/$DEV_BASE/queue/scheduler" 2>/dev/null || \
                echo "noop" > "/sys/block/$DEV_BASE/queue/scheduler" 2>/dev/null && \
                    log_opt "SSD $DEV_BASE I/O 调度器设为 none/noop"
            else
                # HDD: 使用 mq-deadline 或 bfq
                echo "mq-deadline" > "/sys/block/$DEV_BASE/queue/scheduler" 2>/dev/null || \
                echo "bfq" > "/sys/block/$DEV_BASE/queue/scheduler" 2>/dev/null && \
                    log_opt "HDD $DEV_BASE I/O 调度器设为 mq-deadline/bfq"
            fi
        fi
        ...
        new_opts=$(echo "$current_opts" | sed -E 's/\b(noatime|nodiratime|relatime|strictatime|lazyatime)\b,?//g; s/,$//; s/,,+/,/g')
        
        # 构建优化基础选项
        base_opts="noatime,nodiratime"
        if [ "$REMOVABLE" = "1" ]; then
            # USB 设备额外添加 sync 以提高数据安全性
            base_opts="${base_opts},sync"
        fi
