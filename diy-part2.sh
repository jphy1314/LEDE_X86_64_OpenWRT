...
#
# 根因分析：
# 原作者在 reload_service() 中错误使用了 @service[0] 查询配置，
# 导致 enabled=0 时读取失败，无法进入 stop 分支杀进程。
# ==============================================================================
...
start() {
    ...
    while read -r dev mp fs _; do
        ...
        case "$fs" in ext4|btrfs|xfs|f2fs|vfat|exfat|ntfs*|fuseblk) ;; *) continue ;; esac
...
start() {
    ...
    for i in /sys/class/net/*; do
        iface=$(basename "$i")
        case "$iface" in lo|br-*|docker*|veth*|ifb*|tun*|tap*|wg*) continue ;; esac
...
case "$MOUNTPOINT" in /|/rom|/overlay|/boot) exit 0 ;; esac

if ! grep -q " $MOUNTPOINT " /proc/mounts 2>/dev/null; then exit 0; fi
...
#!/bin/sh
# ----------------------------------------------------------------------------
# 1. 禁用原生 strongSwan 自动启动。
# 如果首次启动时原生 ipsec 已经运行，则立即停止以释放 500/4500 端口。
# ----------------------------------------------------------------------------
...
# 原生 ipsec 停止后重新建立正确的 charon 实例，并同步 procd 状态。
# ----------------------------------------------------------------------------
...
protect_system_device() {
    ...
    # 保护 GPT/BIOS 保留分区
    case "$dev" in
        sd[[:alnum:]]*128|nvme[0-9]*p128|mmcblk[0-9]*p128) return 0 ;;
    esac
